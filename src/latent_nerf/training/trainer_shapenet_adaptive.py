...
class Trainer:
    ...
    def train(self):
        ...
        while self.train_step < self.cfg.optim.iters:
            ...
            for data in self.dataloaders['train']:
                ...
                with torch.cuda.amp.autocast(enabled=self.cfg.optim.fp16):
                    ####
                    if self.train_step > 100 and (self.train_step%200 > 100):
                        shape_loss = False
                    else:
                        shape_loss = True
                    ###
                    pred_rgbs, pred_ws, loss = self.train_render(data, shape_loss)
    ...
    def train_render(self, data: Dict[str, Any], shape_loss=True):
        ...
        # Shape loss
        if 'shape_loss' in self.losses and shape_loss:
