...
class Trainer:
    ...
    def init_losses(self) -> Dict[str, Callable]:
        ...
        if self.cfg.optim.lambda_shape > 0 and self.cfg.guide.shape_path1 is not None \
                and self.cfg.guide.shape_path2 is not None:
            ###
            #from src.latent_nerf.training.losses.shape_loss import ShapeLoss
            from src.latent_nerf.training.losses.shape_loss_two_shapes import ShapeLoss
